...
The endpoint instructions provide the pinned helper manifest URL, the SHA-256 of the exact manifest bytes, the helper asset SHA-256, supported operations, and protocol loop. For today's Apple-device path, the fresh client needs Node.js 24 and downloads the single portable `clockchain-agent-handshake.cjs` bundle. It writes the two advertised hashes into local checksum files, requires `shasum -a 256 -c` to report `OK` for the manifest and helper, and only then executes the bundle with `node`. Printing a hash without checking it is not sufficient. The local helper creates the role key, commits the exact local policy, registers a fresh ERC-8004 identity when the Initiator requires it, signs only policy-approved bytes, and verifies the final host certificate. The MCP server never receives a private key.
