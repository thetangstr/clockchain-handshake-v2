...
5. Writes a minimal mode-`0600` `config.yaml` with the feature-detected config version, model `MiniMax-M3`, provider `minimax-cn`, `agent.max_turns: 500`, no fallbacks, memory and user profile disabled, terminal local/profile-home mode, no shell init files, no bashrc sourcing, no hooks, secret redaction enabled, and exactly one enabled HTTP MCP server.
...
7. Hermes is launched twice concurrently from the two workspaces with absolute binary/prompt/usage paths; `-z`, `--ignore-rules`, `--provider minimax-cn`, `-m MiniMax-M3`, and `-t terminal,file,clockchain` are present; resume/continue/safe-mode/profile/skills/worktree flags are absent. “Exact five tools” means the five Clockchain MCP tools; the terminal and file built-ins remain intentionally available so a blank agent can clone, install, and call its wallet bridge.
...
- Read the reusable inference key from one explicit mode-`0600` file outside the repository or one explicit environment variable. Never search the filesystem for it. Accept exactly the pinned `MINIMAX_CN_API_KEY` name.
