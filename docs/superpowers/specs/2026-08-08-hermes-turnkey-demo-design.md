...
For each run, the launcher creates two never-before-used Hermes profiles and two empty workspaces. It does not clone any existing profile. Each profile is created with `--no-skills` and launched with `--ignore-rules`, without `--resume` or `--continue`.

Before either task prompt is delivered, the launcher records a sanitized manifest proving:

- unique run-scoped profile and workspace names;
...
- an empty skills directory and `.no-bundled-skills` marker;
- an empty workspace with no repository, lockfile, `node_modules`, virtualenv, or package cache;
- distinct Clockchain MCP token hashes;
- no wallet file yet;
- no inherited MCP servers before onboarding.

The launcher then injects only the inference credential and one Clockchain MCP connection into each profile. It sets profile-scoped terminal home, disables shell-startup sourcing, disables memory/user-profile features, assigns separate package caches, and supplies one role-specific task. This proves blank Hermes context and blank task workspaces. It is not represented as a hardware or kernel isolation boundary.
...
Mac mini
  ...
  one-command clean-room launcher
    ├── fresh Payer profile + empty workspace + MCP token A + wallet A
    └── fresh Requestor profile + empty workspace + MCP token B + wallet B
...
The launcher fails closed before prompting agents if either freshness manifest is non-empty, profile creation used a clone option, the two tokens are equal, the workspaces overlap, provider/MCP setup is incomplete, or an old profile name is requested.

An agent failure, timeout, role collision, invalid signature, registration failure, MCP authorization failure, certificate failure, or missing evidence ends the run without claiming authorization. The evidence bundle retains sanitized logs, manifests, token hashes, profile names, session id, public addresses, agent ids, and certificate/receipt references. Secrets and wallet files are excluded.

Cleanup is explicit and recoverable for evidence: transcripts and sanitized manifests are retained in the run evidence directory; disposable Hermes profiles, workspaces, wallet files, and per-agent caches are removed after capture.
...
- Launcher tests cover no-clone commands, empty-manifest gates, distinct profiles/workspaces/tokens/caches, MCP allowlist, sanitized evidence, concurrent launch, fail-closed cleanup, and old-profile refusal.
...
1. Capture both pre-prompt clean-room manifests.
2. Mint two distinct production MCP tokens.
...
9. Capture post-run dependency/profile manifests and remove both disposable profiles/workspaces.
...
- Install the launcher under a dedicated Mac-mini path and expose it as one local command; keep secrets in mode-`0600` files outside the repository.
