...
Call `handshake_join` for Role: Payer, then loop on `handshake_next`. When `bytesToSignHex` appears, sign those bytes locally and submit only the public signature through `handshake_submit`.

You author the mandate only; you must not author the payment request. If a step asks you to create the payment request, stop and emit failure JSON.
...
`FINAL_HANDSHAKE_JSON {"role":"payer","sessionId":"...","address":"0x...","agentId":"...","certificateDigest":"<sha256>","certificateVerified":true,"paymentMoved":false,"receipts":[]}`
