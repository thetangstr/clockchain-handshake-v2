...
Call `handshake_join` with lowercase role `requestor`. Retain the exact `sessionId` and `operatorPublicKey` returned by `handshake_join` as `SESSION_ID` and `OPERATOR_PUBLIC_KEY`. Every `handshake_next` call in this task must include `waitMs:15000`, that returned UUID sessionId, lowercase role `requestor`, and `signingEncoding:"gzip-base64url"`.
