...
#   COMPLETION_SOCKET_DEADLINE_MS (src/harness/verified-release-action-recorder.mjs)
#     — the private adapter-completion roundtrip bound; machine-paced in-process
#     socket exchange between the authorization executable and the harness, not
#     a step a human or model waits on
...
for CODE in RENDEZVOUS_UNAVAILABLE EXPIRED MISSING DUPLICATE REORDERED MALFORMED \
            ...
            RATE_BLOCKED AMOUNT_UNRESOLVED FAILED \
...
FROZEN='RENDEZVOUS_UNAVAILABLE|EXPIRED|MISSING|DUPLICATE|REORDERED|MALFORMED|AMBIGUOUS_WRITE|BINDING_MISMATCH|ANCHOR_UNVERIFIED|ROLE_ALREADY_BOUND|RATE_BLOCKED|AMOUNT_UNRESOLVED|FAILED|REHEARSAL_NOT_AUTHORIZABLE|REHEARSAL_SUBJECT_MISMATCH'
