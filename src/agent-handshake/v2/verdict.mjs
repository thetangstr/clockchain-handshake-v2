...
export async function verifyAgentHandshakeV2Authorization({
  ...
  if(terms.identityPolicy.erc8004!=="not_required"){
    ...
    for(const role of ["initiator","responder"]){
      let resolved;try{resolved=await resolveRegistration(parties[role].erc8004.agentId,{expectedOwner:parties[role].sessionKeyAddress,registrationBlock:parties[role].erc8004.registrationBlock});}catch{invalid();}
