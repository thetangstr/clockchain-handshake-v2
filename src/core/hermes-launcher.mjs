...
Do not announce success in prose. The independent checker decides the verdict. FINAL_HANDSHAKE_JSON is success-only. Do not emit FINAL_HANDSHAKE_JSON until handshake_get_certificate has successfully returned and locally verified certificate data with a nonempty 64-lowercase-hex certificateDigest. The final nonempty stdout line must be exactly one ${TERMINAL_MARKER} marker followed by compact JSON:
