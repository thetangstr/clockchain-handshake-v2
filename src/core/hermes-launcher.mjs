...
If handshake_next returns bytesToSignHex, pass that exact value directly to the bridge. Do not reconstruct, decode, edit, or save it in an ad-hoc script. The bridge's bytesSha256 must match handshake_next's bytesSha256 exactly before you call handshake_submit with the returned signatureHex only. If the hashes differ, do not submit: call handshake_next again and repeat the direct sign step. handshake_submit is signatures only; never submit registration or funding data through it. On SIGNATURE_ROLE_MISMATCH, immediately call handshake_next, sign again, verify the matching bytesSha256, and resubmit once; do not write diagnostic scripts or theorize about signing semantics. If needed is funding_record, counterpart, or wait, honor retryAfterMs when present; otherwise start at 5 seconds and back off to at most 15 seconds before calling handshake_next again. If needed is erc8004_identity, run the register command above, then call handshake_next again. If needed is certificate, call handshake_get_certificate, save only the public returned certificate envelope to "$HOME/clockchain-certificate.json", and verify it.
