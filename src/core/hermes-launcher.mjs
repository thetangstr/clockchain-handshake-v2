...
function staticPrompt({ role: cleanRole }) {
  ...
  const dependencyWait = cleanRole === "payer"
    ? "If needed is funding_record, requestor_identity_ready, handshake_required, clockchain_confirmation, counterpart_transition, counterpart, or wait, this is a normal dependency wait, not an instruction to author another role's artifact. requestor_identity_ready means wait for the Requestor; honor retryAfterMs when present, otherwise start at 5 seconds and back off to at most 15 seconds, then call handshake_next again."
    : "If needed is funding_record, payer_mandate, handshake_required, clockchain_confirmation, counterpart_transition, counterpart, or wait, this is a normal dependency wait, not an instruction to author another role's artifact. payer_mandate means wait for the Payer; honor retryAfterMs when present, otherwise start at 5 seconds and back off to at most 15 seconds, then call handshake_next again.";
