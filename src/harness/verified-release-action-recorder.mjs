...
function verifyReleaseAssetBytes({ helperBytes, manifestBytes, manifestDigest }) {
  if (!Buffer.isBuffer(manifestBytes) || manifestBytes.length < 1) fail();
  ...
  if (
    ...
    manifest.version !== AGENT_HANDSHAKE_HELPER_VERSION ||
    typeof manifest.nodeRuntime !== "string" ||
    !new RegExp(`^${AGENT_HANDSHAKE_HELPER_NODE_MAJOR}\\.`).test(manifest.nodeRuntime) ||
  ...
  if (
    ...
    typeof asset.byteLength !== "string" || !DECIMAL.test(asset.byteLength)
  ) fail();
  if (!Buffer.isBuffer(helperBytes) || helperBytes.length < 1 || helperBytes.length > 1024 * 1024) fail();
  if (
    String(helperBytes.length) !== asset.byteLength ||
    createHash("sha256").update(helperBytes).digest("hex") !== asset.sha256
  ) fail();
}

async function fetchReleaseAsset(fetchAsset, url, maxBytes, stage) {
  try {
    const bytes = Buffer.from(await fetchAsset(url));
...
export async function preloadVerifiedReleaseAssets({ fetchAsset, manifestDigest, releaseAssets, workspace }) {
  if (!SHA256.test(manifestDigest)) fail();
  let manifestBytes;
  let helperBytes;
  if (releaseAssets !== undefined) {
    manifestBytes = Buffer.isBuffer(releaseAssets?.manifestBytes) ? releaseAssets.manifestBytes : null;
    helperBytes = Buffer.isBuffer(releaseAssets?.helperBytes) ? releaseAssets.helperBytes : null;
    if (manifestBytes === null || helperBytes === null) fail();
  } else {
    if (typeof fetchAsset !== "function") fail();
    manifestBytes = await fetchReleaseAsset(fetchAsset, `${RELEASE_PREFIX}manifest.json`, 64 * 1024, "release-manifest-fetch");
    helperBytes = await fetchReleaseAsset(fetchAsset, `${RELEASE_PREFIX}clockchain-agent-handshake.cjs`, 1024 * 1024, "release-helper-fetch");
  }
  verifyReleaseAssetBytes({ helperBytes, manifestBytes, manifestDigest });
...
export async function createVerifiedReleaseActionRecorder(input = {}) {
  ...
  try {
    options = snapshotObject(input, [
      "actionTtlMs", "completionDeadlineMs", "fetchAsset", "manifestDigest", "platform",
      "releaseAssets", "room", "runtimeExecPath", "socketRoot",
  ...
  const fetchAsset = options.fetchAsset ?? null;
  ...
  try {
    await preloadVerifiedReleaseAssets({
      fetchAsset,
      manifestDigest,
