...
const PARTY_FAILURE_STAGE = /^(?:runtime-create|exchange-create|bootstrap-publish|bootstrap-await|exchange-destroy|managed-hold|runtime-run(?:\.(?:peer-validate|listener-create|listener-ready|invitation-await|recorder-create|recorder-(?:construction-(?:options|room|paths|platform)|release-(?:manifest|helper)-fetch|release-assets|adapter-layout|completion-socket)|checkpoint-client-create|bridge-create|provider-auth(?:-(?:input|decode|parse|install|export))?|transport-create|adapter-create|agent-starting|agent-launch(?:-(?:spawn|stream|initialize|session|model|prompt|completion(?:-(?:protocol(?:-(?:envelope(?:-(?:runtime|session-id|update-type|before-session|early-tool|provisional-session|active-session))?|usage|tool-result|bridge|retained|event))?|permission|stop))?))?|evidence-validate|certificate-event|agent-terminate|evidence-collect|teardown|listener-listen-(?:eacces|eaddrinuse|eaddrnotavail|eperm|other)))?)$/;
