...
const PARTY_FAILURE_STAGE = /^(?:runtime-create|exchange-create|bootstrap-publish|bootstrap-await|exchange-destroy|managed-hold|runtime-run(?:\.(?:peer-validate|listener-create|listener-ready|invitation-await|recorder-create|recorder-(?:construction-(?:options|room|paths|platform)|release-(?:manifest|helper)-fetch|release-assets|adapter-layout|completion-socket)|checkpoint-client-create|bridge-create|provider-auth(?:-(?:input|decode|parse|install|export))?|transport-create|adapter-create|agent-starting|agent-launch|evidence-validate|certificate-event|agent-terminate|evidence-collect|teardown|listener-listen-(?:eacces|eaddrinuse|eaddrnotavail|eperm|other)))?)$/;
