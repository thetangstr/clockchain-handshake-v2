...
const prefix = "https://github.com/thetangstr/clockchain-handshake-v2/releases/download/v2.1.8/";
...
test("tracks the independently published helper in a separate post-release pin", async () => {
  ...
  assert.deepEqual(pin, {
    version: "2.1.8",
    sourceCommit: "372f85699f96258399c7c02c50bcc4559295f412",
    manifestDigest: "956c8d94d4e7dbbaad5247e8de3f958ad75ec81831193c31c995e3787bf33bf7",
...
test("the published post-release pin and its schema stay internally consistent", async () => {
  // pin.json records the LAST PUBLISHED release (v2.1.8, measured from the
...
test("rejects unknown keys, duplicates, redirects, digest drift, and native executable substitutions", () => {
  ...
  const mutations = [
    ...
    // A "none" signature claiming verified:true is exactly the decorative
    // claim a reviewer flagged — the schema now forbids it.
    { ...base, assets: base.assets.map((entry) => ({ ...entry, nativeSignature: { ...entry.nativeSignature, verified: true } })) },
