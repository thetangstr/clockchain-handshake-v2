...
test("harness adapter executes the exact MCP-bound argv after only a short digest approval", async (t) => {
  ...
  for (let attempt = 0; attempt < 100 && (mcpOutput.match(/\n/g) ?? []).length < 2; attempt += 1) {
  ...
  assert.deepEqual(requested, [
    "https://github.com/thetangstr/clockchain-handshake-v2/releases/download/v2.1.3/manifest.json",
    "https://github.com/thetangstr/clockchain-handshake-v2/releases/download/v2.1.3/clockchain-agent-handshake.cjs",
  ]);
...
test("harness adapter submits the private proposal checkpoint before releasing the helper signature", async (t) => {
  ...
  const helperSource = [
    ...
    `process.stdout.write(JSON.stringify({schema:"clockchain.agent-handshake-cli-result/v1",helperVersion:"2.1.3",operation:"sign",address:${JSON.stringify(account.address)},bytesSha256:${JSON.stringify(fixture.request.bytesSha256)},signatureHex:${JSON.stringify(signatureHex)}})+"\\n");`,
  ].join("");
  const helperDigest = createHash("sha256").update(helperSource).digest("hex");
  const manifest = JSON.stringify({
    schema: "clockchain.agent-handshake-release-manifest/v1",
...
test("accepts inert leading narration comments before one exact Claude approval marker", async (t) => {
  const parent = await mkdtemp(join(tmpdir(), "fresh-agent-claude-commented-approval-"));
  t.after(() => rm(parent, { recursive: true, force: true }));
  const children = {};
  const command = nonterminalHelperCommand("responder", "init");
  ...
  const spawnProcess = () => {
    ...
    child.stdin = { end() {
      queueMicrotask(() => {
        ...
        children.responder.stdout.emit("data", Buffer.from(claudeHelperProofEvent(
          nonterminalHelperResult("responder", "init"),
          { command: commentedApproval, id: "commented-approval" },
        )));
        children.responder.stdout.emit("data", Buffer.from(claudeHelperProofEvent(helperProof("responder"))));
        children.initiator.emit("close", 0, null);
        children.responder.emit("close", 0, null);
